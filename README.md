...
| 对照评测 | 单次页面预实验均为 11/11；正式固定变更执行 A/B 各 3 次均通过，未观察到成功率提升 |
...
- [对照评测运行册](evaluation/T7_对照评测运行册.md)与[正式重复固定变更对照](evaluation/formal-ab-20260921/README.md)
...
正式重复固定变更执行对照已完成：同一公开案例、同一模型及时间/工具预算下 A/B 各 3 次，独立 VC 探针均为 3/3，未观察到成功率提升；没有 Token/金额硬上限，B 的合同上下文更长，因此不是严格等 Token/费用实验。该结果不覆盖完整工作流或未见任务。仍需补齐未见案例，以及文件写入或 shell 子进程执行中的页面取消/强制中断实验。Electron 在 0 次工具调用阶段的真实 `SIGKILL` 恢复已通过；自动化另验证停止信号会终止真实前台 shell 进程组、保留停止前产物，并验证孤儿运行转为 interrupted 后可显式重试。这些结果不代表全部 UI 或生产环境正确。示例事件使用模拟身份，不支持真实业务转化率结论。
