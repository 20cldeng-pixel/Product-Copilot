...
仍需补齐正式重复配对评测、未见案例，以及文件写入或 shell 子进程执行中的页面取消/强制中断实验。Electron 在 0 次工具调用阶段的真实 `SIGKILL` 恢复已通过；自动化另验证停止信号会终止真实前台 shell 进程组、保留停止前产物，并验证孤儿运行转为 interrupted 后可显式重试。这些结果不代表全部 UI 或生产环境正确。示例事件使用模拟身份，不支持真实业务转化率结论。
