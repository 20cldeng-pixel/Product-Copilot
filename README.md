...
正式重复固定变更执行对照已完成：公开 Gather 案例 A/B 均为 3/3。另完成作者自建的独立未见 DeskFlow 案例：原始需求核心两组仍均为 3/3；扩展合同 A 为 0/3、B 为 3/3，但只差 schema 升版与新版修复，而数据结构没有变化，因此不能把该差异解释为同一用户任务的开发成功率提升。B 的平均时长、工具调用和未核账 cost 估算更高；两轮实验都没有 Token/金额硬上限，也不覆盖完整工作流。仍需补齐由共同目标必然导出隐藏判据的复杂未见案例，以及文件写入或 shell 子进程执行中的页面取消/强制中断实验。Electron 在 0 次工具调用阶段的真实 `SIGKILL` 恢复已通过；自动化另验证停止信号会终止真实前台 shell 进程组、保留停止前产物，并验证孤儿运行转为 interrupted 后可显式重试。这些结果不代表全部 UI 或生产环境正确。示例事件使用模拟身份，不支持真实业务转化率结论。
