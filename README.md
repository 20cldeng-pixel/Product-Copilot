...
- **不弄脏你的机器，也不给你设路障**——标准模式把依赖、缓存、临时文件全部重定向到项目专属运行区（`HOME`/`TMPDIR`/npm、pip、cargo 缓存），删项目即干净、多项目互不串扰；需要 `gh`、`git push` 这类依赖本机登录态的操作时切完全访问；跑来源不明的项目时还能切到叠加系统级沙盒的**受限模式**。系统核心、提权与「会自动执行代码的持久化配置」（MCP 配置、开机自启、模型设置）在三档下都禁止改写
...
- **权限模式**——「受限 / 标准 / 完全访问」三档：受限模式叠加系统级沙盒（适合跑来源不明的项目，代价是浏览器与浏览器自动化不可用）；标准模式（默认）把开发环境隔离在项目运行区，不污染全局、读不到你的凭据；完全访问用你的真实环境执行，`gh`、`git push`、浏览器与 Playwright 都能用，切换前有一次风险确认。系统核心、提权与自动执行代码的持久化配置在三档下都禁止改写
...
命令的放行由 EasyMint 在**执行前判定**；边界分三类，任何会话设置都改不了：

- **系统核心**——系统目录、原始设备，以及磁盘与安全机制本身（`csrutil`、`spctl`、`fdesetup` 等）：两种模式都拒绝
- **危险操作**——提权（`sudo` / `doas`）、系统服务变更，以及**会自动执行代码的持久化配置**（MCP 配置、开机自启目录、EasyMint 的模型与供应商设置）：两种模式都拒绝
- **普通资源**——标准模式限定在项目工作区与项目专属运行区内（越界写入、读高敏凭据会被拒）；完全访问不做限定

三档的差别在**环境与强制方式**，不在能不能干活：

- **受限模式**——在标准模式的全部边界之上再叠加**系统级沙盒**（macOS 用系统自带的 Seatbelt，Linux 用 bubblewrap，Windows 用独立沙盒账户配合内核级网络过滤）。边界由操作系统强制，脚本与变量也绕不过去。代价必须说清：**浏览器与浏览器自动化（Playwright）不可用**（Chromium 无法在已沙盒的进程里自建沙盒）、**无法管理其它命令启动的进程**、`open` 打不开——它面向"跑来源不明的项目 / 安装脚本"，不是日常开发档
- **标准模式（默认）**——开发环境隔离在项目运行区：`HOME`、`TMPDIR`、npm / pip / cargo 等缓存全部重定向到 `~/.easymint/runtimes/<项目>/`，不污染全局环境、多项目互不串扰、删项目即干净。代价是依赖本机登录态的工具（`gh`、`git push`、`gcloud`）在此模式下用不了——需要它们时切完全访问
- **完全访问**——用你的真实环境执行：`gh`、`git push`、系统命令、打开浏览器与浏览器自动化（Playwright）都能用；切换前有一次性的风险确认
- **判定口径克制**——只对确定目标的路径做提前判定，并拦截提权类命令；命令中的消息文本、正则与脚本内容不会被当成路径误判。需要说明的是：标准与完全访问档走的是**执行前判定**而非 OS 级强制，变量与脚本内部的动态路径可以绕过；需要绕不过去的边界时用受限模式
- **状态一眼可见**——输入卡的盾形图标与配色随权限档位切换：受限（盾内锁）、标准（盾内勾）、完全访问（盾内感叹号 + 危险色）
