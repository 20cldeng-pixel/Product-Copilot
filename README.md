...
- **不弄脏你的机器，也不给你设路障**——标准模式把依赖、缓存、临时文件全部重定向到项目专属运行区（`HOME`/`TMPDIR`/npm、pip、cargo 缓存），删项目即干净、多项目互不串扰；同时命令跑在**系统级沙盒**里（macOS Seatbelt / Linux bubblewrap / Windows 独立沙盒账户），越界写入与凭据读取由内核拦下、脚本与变量绕不过去。出网按**域名白名单**放行，浏览器与容器类命令走**精确豁免**，所以日常开发不受影响；需要 `gh` 这类依赖本机登录态配置的工具时切完全访问（`git push` 走 ssh-agent 通道，标准模式直接可用）；只想让它读、不让它动手时切到**只读模式**。系统核心、提权与「会自动执行代码的持久化配置」（MCP 配置、开机自启、模型设置）在三档下都禁止改写
...
- **权限模式**——「只读 / 标准 / 完全访问」三档：只读模式可以自由读取，但不执行任何命令、不写文件、不联网（适合审阅来源不明的项目，代价是不能构建/测试/装依赖）；标准模式（默认）把开发环境隔离在项目运行区，命令跑在系统级沙盒里、出网按域名白名单放行，`git push` 走 ssh-agent 可直接用；完全访问用你的真实环境执行、不套沙盒，`gh`、浏览器与 Playwright 都能用，切换前有一次风险确认。系统核心、提权与自动执行代码的持久化配置在三档下都禁止改写
...
- **只读模式**——**读自由，其余一律拒绝**：不执行任何命令、不写入任何文件、不启用 MCP、不联网（`web_fetch` / `web_search` 也停用——它们本身就是外泄出口）。它的保证是**结构性的**：整个执行面被移除 ⇒ 没有进程能发起网络请求 ⇒ 读到敏感内容也送不出去，所以这一档**允许**读取凭据。代价必须说清：不能构建 / 测试 / 装依赖，**git 操作也不行**——它面向"只看不动"（审阅陌生项目 / 不放心它动手时），不是日常开发档
- **标准模式（默认）**——两层一起生效。**环境隔离**：`HOME`、`TMPDIR`、npm / pip / cargo 等缓存全部重定向到 `~/.easymint/runtimes/<项目>/`，不污染全局环境、多项目互不串扰、删项目即干净。**内核边界**：命令跑在系统级沙盒里（macOS Seatbelt / Linux bubblewrap / Windows 独立沙盒账户），越界写入与凭据读取由内核拦下——脚本与变量绕不过去，这与"判定层被绕过"是不同的强度。配套三件事让它不误伤：① **兼容性清单**（PTY、交互式工具等沙盒默认会挡、而开发必需的能力）；② **精确豁免**（`open` / Playwright / `docker` 这类必须自建沙盒、在系统沙盒内结构性跑不了的命令，改在沙盒外执行，并在结果里标出"沙盒豁免"）；③ **网络白名单**（主流包管理器、代码托管与常见开发端点已在列；未知域名被拦下，可在设置或项目内 `.easymint/sandbox.json` 追加）。代价是读本机登录态配置的工具（`gh`、`gcloud`）在此模式下用不了——需要它们时切完全访问；**`git push` / `fetch` 可以直接用**（走 ssh-agent 签名，私钥不进会话环境，前提是 key 已在 agent 里：`ssh-add`）。
- **完全访问**——用你的真实环境执行、**不套沙盒**：`gh`、`git push`、系统命令、打开浏览器与浏览器自动化（Playwright）都能用；切换前有一次性的风险确认。上面三类底线仍然生效，但只剩执行前判定这一层
