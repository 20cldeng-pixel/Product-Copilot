 ...
 *  - DMG 卷图标（assets/icon-volume.icns）：同为「自带形状」口径——dmgbuild 只把 dmg.icon
 *    原样拷成卷根 `.VolumeIcon.icns`，Finder 画桌面那个盘符时**不套任何形状**（同本文件
 *    的运行时图标一类）。五条产物由 scripts/gen-appicon.py 生成（icns 两条需 --icns），
 *    改素材后重跑该脚本。
 *
