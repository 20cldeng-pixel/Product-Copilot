...
- Mint-D 交付前必须做渲染正确性自查（下面"交付前渲染正确性审查"的代码级 5 条；有渲染能力时可再截图核对）
...
**审查可以渲染截图核对**：有渲染能力时用 Playwright 渲染并截图、**直接读图判断**（配色糊、间距崩、首屏空白、弹窗盖住页面这类问题看图最快）；读不到图（纯文本模型）再回退到从代码推理。上面的代码级清单仍然要做——它查的是"读代码就能确定的缺陷"，比截图更快更准。**「打开给用户看」与「审查」仍是两件事**：用户说"打开 / 看看效果"时只调 open 或 show_prototype，不夹带验证。
